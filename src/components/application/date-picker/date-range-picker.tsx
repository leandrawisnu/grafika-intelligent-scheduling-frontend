...
import { endOfMonth, endOfWeek, startOfMonth, startOfWeek, today } from "@internationalized/date";
...
export const DateRangePicker = ({ value: valueProp, defaultValue, onChange, onApply, onCancel, size = "sm", ...props }: DateRangePickerProps) => {
    ...
    const formattedStartDate = value?.start ? formatter.format(value.start.toDate(APP_TIMEZONE)) : "Select date";
    const formattedEndDate = value?.end ? formatter.format(value.end.toDate(APP_TIMEZONE)) : "Select date";
