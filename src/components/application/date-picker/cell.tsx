...
import { getDayOfWeek, isToday } from "@internationalized/date";
...
export const CalendarCell = ({ date, isHighlighted, showOutOfRangeDates = false, ...props }: CalendarCellProps) => {
    ...
    const isTodayDate = isToday(date, APP_TIMEZONE);
        ...
        <AriaCalendarCell
            ...
            className={({ isDisabled, isFocusVisible, isSelectionStart, isSelectionEnd, isSelected, isOutsideMonth }) => {
                ...
                return cx(
                    ...
                    isInRange && isDisabled && "bg-muted",
                    isSelected && isRangeCalendar && "bg-muted",
        ...
        >
            {({ isDisabled, isFocusVisible, isSelectionStart, isSelectionEnd, isSelected, formattedDate }) => {
                ...
                return (
                    <div
                        className={cx(
                            "relative flex size-full items-center justify-center rounded-full text-sm text-foreground hover:text-foreground",
                            ...
                            !isSelected && !isDisabled ? "hover:bg-muted hover:font-medium" : "",
                            !isSelected && isTodayDate ? "bg-muted font-medium hover:bg-muted/80" : "",
                        )}
                    >
                        {formattedDate}
                        ...
                        {(isHighlighted || isTodayDate) && (
                            <div
                                className={cx(
                                    ...
                                    markedAsSelected ? "bg-primary-foreground" : "bg-primary",
