    ...
    return (
        <AriaGroup
            ...
            className={({ isFocusWithin, isDisabled, isInvalid }) =>
                cx(
                    "group/input relative flex w-full flex-row place-content-center place-items-center rounded-lg border border-input bg-background shadow-xs transition-shadow duration-100 ease-linear",

                    isFocusWithin && !isDisabled && "ring-2 ring-ring ring-offset-1",
            ...
            <AriaDateInput {...inputProps} className={cx("flex w-full", sizes[size].root, typeof inputProps.className === "string" && inputProps.className)}>
                {(segment) => (
                    <AriaDateSegment
                        segment={segment}
                        className={cx(
                            "rounded px-0.5 text-foreground tabular-nums caret-transparent focus:bg-primary focus:font-medium focus:text-primary-foreground focus:outline-hidden",
